-- ============================================================================
-- Fix is_upcoming Column - Create Column and Triggers
-- ============================================================================
--
-- Problem: is_upcoming column doesn't exist in events table
-- Solution: Create the column, then set up triggers to maintain it
--
-- Run this in your Supabase SQL Editor
-- ============================================================================

-- Step 1: Add is_upcoming column to events table
ALTER TABLE events
  ADD COLUMN IF NOT EXISTS is_upcoming BOOLEAN DEFAULT false;

-- Add index for better performance
CREATE INDEX IF NOT EXISTS idx_events_is_upcoming
  ON events(user_id, is_upcoming)
  WHERE is_upcoming = true;

-- Step 2: Create a function to recalculate is_upcoming for a user's events
...
-- Step 3: Create a trigger function to recalculate after insert/update
...
-- Step 6: Also recalculate when events are deleted
...
-- Step 7: Recalculate is_upcoming for ALL existing events
...
-- ============================================================================
-- After running this:
-- 1. Only ONE event per user will have is_upcoming = true
-- 2. That event will be the closest one to now that hasn't ended
-- 3. Triggers will automatically maintain this as you create/update events
-- ============================================================================
